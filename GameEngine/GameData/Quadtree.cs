namespace GameEngine.GameData
{
    ...
    public class Quadtree<TType> where TType : QuadtreeItem
    {
        ...
        public IEnumerable<QuadtreeNode<TType>> GetLeafNodes(Rectangle rectangle)
        {
            return GetRoot().GetLeafNodes(rectangle);
        }

        public void Insert(Vector2 pos, TType value, bool ForseOptimize = true, bool plus1 = true, bool plusTileSize = true)
        {
            Vector2 newpos = pos;

            if (plusTileSize)
                newpos = new Vector2(pos.X * Tile.tileSize, pos.Y * Tile.tileSize);
            if (plus1)
        ...
        public TType Get(Vector2 pos, bool TilePosStyle = true)
        {
            try
            {
                ...
                if (TilePosStyle)
                    position = new Vector2(pos.X * Tile.tileSize, pos.Y * Tile.tileSize) + new Vector2(1, 1);
                else
                    position = pos;

                if (Utilites.Intersect(new Rectangle(new Point(0, 0), new Point((int)node.Size, (int)node.Size)), position))
                {
            ...
            if (tree.Nodes != null)
            {
                foreach (var node in tree.Nodes)
                {
                    if (node.HasNotNullLeaf())
                        if (Physics.CheckCollision(Camera.position, new Vector2f(RenderCore.window.Size.X, RenderCore.window.Size.Y), node.Transform, new Vector2(node.Size, node.Size)))
                        {
                            if (node.IsLeaf())
                            {
                                RenderSystem.DrawCollisionSquare(node.Transform, new Vector2(node.Size, node.Size), Color.Blue);

                                if (node.Data != null)
                                {
                                    RenderCore.DrawCollisionSquare(node.Transform + new Vector2(1, 1), new Vector2(node.Size - 2, node.Size - 2), Color.Red);
                                }
                            }
                            else
        ...
        public class QuadtreeNode<TType> where TType : QuadtreeItem
        {
            ...
            public QuadtreeNode<TType> Search(Vector2 targetPos)
            {
                try
                {
                    if (depth == 0)
            ...
            public QuadtreeNode<TType> Subdivide(Vector2 targetPos, TType value, int depth = 0)
            {
                if (depth == 0)
                {
                    //Optimize();
            ...
            public int GetIndexOfPosition(Vector2 lookupPos, Vector2 nodePos)
            {
                int index = 0;
            ...
            public IEnumerable<QuadtreeNode<TType>> GetLeafNodes(Rectangle rectangle)
            {
                if (IsLeaf())
                {
                    ...
                    if (Rectangle.Intersect(rectangle, new Rectangle(position.X, position.Y, (int)size, (int)size)))
                        yield return this;
                    else
                        yield return null;
                }
